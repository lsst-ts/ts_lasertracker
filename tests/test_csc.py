...
class CscTestCase(salobj.BaseCscTestCase, unittest.IsolatedAsyncioTestCase):
    ...
    async def quick_power_on(self, laser_warmup_time: float, wait_warmup: bool) -> None:
        ...
        assert self.csc.model.mock_t2sa is not None
        self.csc.model.mock_t2sa.laser_warmup_time = laser_warmup_time

        await self.remote.cmd_laserPower.set_start(power=1, timeout=STD_TIMEOUT)

        if wait_warmup:
            await self.csc.model.mock_t2sa.laser_warmup_status
    ...
    async def test_laser_power(self) -> None:
        async with self.make_csc(
            ...
            assert self.csc.model.mock_t2sa.laser_status == "WARM"

            # Again, since there is no event, wait for the warmup task to
            # complete.
            await self.csc.model.mock_t2sa.laser_warmup_status

            assert self.csc.model.mock_t2sa.laser_status == "LON"

            # power off
            await self.remote.cmd_laserPower.set_start(power=0, timeout=STD_TIMEOUT)

            assert self.csc.model.mock_t2sa.laser_status == "LOFF"
            assert self.csc.model.mock_t2sa.laser_warmup_status.done()
    ...
    async def test_set_reference_group(self) -> None:
        ...
        async with self.make_csc(
            ...
            assert self.csc.model.mock_t2sa.reference_frame == "FRAMEM2"
